...
export const compressImagesInBackground = async (qrs: TQrServerData[]) => {
  try {
...
const handleImageCompression = async (qr: TQrServerData) => {
  try {
...
const handleMediaPlaceholder = (qr: TQrServerData) => {
  const typeMap = {
    pdf: "application/pdf",
    video: "video/mp4",
  };
